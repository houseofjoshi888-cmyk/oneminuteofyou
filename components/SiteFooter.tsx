...
export function SiteFooter({ global = false }: { global?: boolean }) {
  ...
  return <footer className="site-footer global-site-footer">
    <div className="footer-brand"><strong>ONE MINUTE OF YOU</strong><p>Sixty seconds of movement become one deterministic artwork.</p><span>© 2026 The House of Joshi. One Minute of You. All rights reserved.</span></div>
    <div><h2>Explore</h2><nav aria-label="Explore"><Link href="/about">About</Link><Link href="/how-it-works">How it works</Link><Link href="/houses">Royal Houses</Link><Link href="/gallery">Collection</Link><Link href="/my-collection">My Collection</Link><Link href="/generate">Create</Link></nav></div>
    <div><h2>Trust</h2><nav aria-label="Trust and support"><Link href="/terms">Terms & Conditions</Link><Link href="/privacy">Privacy Policy</Link><Link href="/security">Security</Link><Link href="/faq">FAQ</Link><Link href="/verify">Verify a seed</Link><Link href="/recovery">Mint recovery</Link><a href="https://thehouseofjoshi.com/contact" target="_blank" rel="noreferrer">Contact ↗</a></nav></div>
