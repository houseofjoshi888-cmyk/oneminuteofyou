...
export default function GeneratePage() {
  ...
  return <main className={result ? "result-page" : "studio"}>
    ...
    {!result ? <Recorder onComplete={complete} /> : <div className="result-grid"><Renderer words={result.words} features={result.features} /><section className="result-copy"><p className="eyebrow"><span /> ONE MINUTE OF YOU · {compositionName(result.hash).toUpperCase()}</p><h2>{artworkName(result.hash, result.features)}<br /><em>is uniquely yours.</em></h2><p className="mint-note">Your seed selected the {compositionName(result.hash)} composition family. Other minutes can become twin blooms, flowing ribbons, halos, nebulae, or vortices.</p><div className="hash">SHA-256 / {result.hash}</div><Stats features={result.features} /><div className="result-actions"><Link className="primary-button" href="/mint">Prepare NFT edition <span>↗</span></Link><button className="secondary-button" onClick={() => setResult(null)}>Record again</button></div></section></div>}
  </main>;
}
