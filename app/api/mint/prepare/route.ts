...
export async function POST(request: Request) {
  ...
  try {
    ...
    const liveHtml = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>One Minute of You · Living Artwork</title><style>*{box-sizing:border-box}html,body{margin:0;width:100%;height:100%;overflow:hidden;background:#020203}img,canvas{position:absolute;inset:0;width:100%;height:100%;object-fit:contain}canvas{background:#020203}</style></head><body><img id="poster" alt="Complete still artwork" src="https://gateway.pinata.cloud/ipfs/${imageCid}"><canvas aria-label="Animated dust formation of the complete artwork"></canvas><script>const c=document.querySelector('canvas'),x=c.getContext('2d'),im=document.querySelector('#poster');let gateway=0;const sources=['https://gateway.pinata.cloud/ipfs/${imageCid}','https://ipfs.io/ipfs/${imageCid}','https://cloudflare-ipfs.com/ipfs/${imageCid}'];im.crossOrigin='anonymous';im.onerror=()=>{if(++gateway<sources.length)im.src=sources[gateway]};im.onload=()=>{c.width=1024;c.height=1024;const b=document.createElement('canvas');b.width=c.width;b.height=c.height;const q=b.getContext('2d');q.drawImage(im,0,0,c.width,c.height);const d=q.getImageData(0,0,c.width,c.height).data,p=[];let s=0x${seed.slice(0,8)};const r=()=>((s=Math.imul(s^s>>>15,1|s),s^=s+Math.imul(s^s>>>7,61|s))^s>>>14)>>>0;for(let i=0;i<26000;i++){let z=(r()%(c.width*c.height))|0,j=z*4;if(d[j]+d[j+1]+d[j+2]>42)p.push([z%c.width,(z/c.width)|0,1+r()%3,d[j],d[j+1],d[j+2]])}let started=performance.now();function f(now){const cycle=((now-started)%23000)/23000,a=cycle<.435?cycle/.435:cycle<.543?1:cycle<.978?1-(cycle-.543)/.435:0,e=a*a*(3-2*a),n=(e*p.length)|0;x.fillStyle='#020203';x.fillRect(0,0,c.width,c.height);for(let i=0;i<n;i++){let v=p[i];x.fillStyle='rgb('+v[3]+','+v[4]+','+v[5]+')';x.fillRect(v[0],v[1],v[2],v[2])}requestAnimationFrame(f)}requestAnimationFrame(f)}</script></body></html>`;
    const animationCid = await pinFile(jwt, new File([liveHtml], "living-artwork.html", { type: "text/html" }), `${seed}-living-artwork.html`);
    metadata.image = `ipfs://${imageCid}`; metadata.animation_url = `ipfs://${animationCid}`; metadata.background_color = "020203";
