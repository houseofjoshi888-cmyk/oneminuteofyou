  ...
  return <main className="result-page" style={houseStyle}>
    ...
    <div className="result-grid">
      ...
      <section className="result-copy">
        ...
        <div className="result-actions"><button className="primary-button" onClick={highRes} disabled={exporting}>{exporting ? "Rendering royal NFT…" : "Export 4096 PNG"}<span>↓</span></button><button className="secondary-button" onClick={vectorExport}>Export SVG</button><button className="secondary-button living-export" onClick={livingLoop} disabled={recordingLoop || !livingCanvas}>{recordingLoop ? "Recording 12-second cycle…" : "Export living WebM"}</button><button className="secondary-button" onClick={() => metadata && exportMetadata(metadata, `${title.toLowerCase().replaceAll(" ", "-")}-${result.hash.slice(0, 8)}.json`)}>Royal metadata</button><button className="secondary-button" onClick={() => exportProvenanceCertificate(result.hash, title, house.name, house.primary, house.secondary)}>Provenance certificate</button></div>
